...
function InboxView() {
  ...
  return <div className="inbox-workspace">
    ...
    <div className="inbox-layout">
      ...
      <section className="panel chat-panel">{conversation ? <><div className="chat-header"><div><h2>{displayIdentity(conversation)}</h2><small>{conversation.is_lid ? 'WhatsApp identity protected' : displayIdentity({ phone: conversation.phone })}{conversation.company ? ` · ${conversation.company}` : ''}</small></div><span className={`campaign-status ${conversation.status === 'human_takeover' ? 'stopped' : conversation.ai_enabled ? 'running' : 'paused'}`}>{conversation.status === 'human_takeover' ? 'Human attention required' : conversation.ai_enabled ? 'AI Active' : 'Human Active'}</span></div><div className="message-stream">{messages.map((item) => <article key={item.id} className={`chat-message ${item.direction === 'inbound' ? 'inbound' : 'outbound'}`}><small>{item.direction === 'inbound' ? 'Customer' : 'Sudarshan Pipes'}</small><p>{item.body}</p><time>{item.status}</time></article>)}</div><div className="chat-controls"><button className="secondary-btn" onClick={toggleAI} disabled={busy}>{conversation.ai_enabled ? 'Pause AI' : 'Resume AI'}</button><button className="secondary-btn" onClick={takeOver} disabled={busy || conversation.status === 'human_takeover'}>Take Over</button><button className="secondary-btn" onClick={resolve} disabled={busy || conversation.status === 'resolved'}>Mark Resolved</button></div><form className="reply-form" onSubmit={sendReply}><textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Write a WhatsApp reply..." aria-label="WhatsApp reply" /><button className="primary-btn" disabled={busy || !draft.trim()}>Send Reply</button></form></> : <div className="empty-preview">Select a conversation to view its history.</div>}</section>
    </div>
  </div>
}
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand">Sudarshan Pipes</div>
        <nav>
          {navItems.map((item) => (
            <button
              key={item}
