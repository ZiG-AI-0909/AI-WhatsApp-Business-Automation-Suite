...
class IncomingMessageService {
    ...
    async process(input, sendMessage, io) {
        ...
        try {
            if (/^(hi|hello|hey)\b[!. ]*$/i.test(incoming.text)) {
                await this._reply(conversation.id, incoming, `Hello! Welcome to Sudarshan Pipes. How can I help you today?`, sendMessage, io);
                return true;
            }
    ...
    _buildSystemPrompt(knowledgeContext) {
        const businessName = process.env.BUSINESS_NAME || 'Sudarshan Pipes';
